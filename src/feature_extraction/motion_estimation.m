function [R,T,lambda] = motion_estimation(matches, E, K)
    %% Preprocessing
    N = size(matches,2);
    x1_ = [matches(1:2,:); ones(1,N)];
    x2_ = [matches(3:4,:); ones(1,N)];
    x1 = K\x1_;
    x2 = K\x2_;
    ...
    for j=1:4     
        ...
        % Take R,T with the lowest number of negative lambdas (ideally 0)
        this_nnz = nnz(d1)+nnz(d2);
        if this_nnz < best_nnz
            best_nnz = this_nnz;
            R = R_cell{j};
            T = T_cell{j};
            lambda = d_cell{j};
