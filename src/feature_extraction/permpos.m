...
%   permutation of M trues and (N-M) falses. Note that each column will hence
%   contain (N-1) trues. 
%   
...
%   File Exhange: PERMN, ALLCOMB, BALLATSQ, NONES, NEXTPERMPOS

% for Matlab R2017a
% version 2.4 (may 2018)
% (c) Jos van der Geest
...
% 2.2 oct 2012 - added reference to NEXTPERMPOS
%              - fixed a few spelling errors in the help
% 2.3 mar 2016 - updated to R2015a
% 2.4 may 2018 - update for R2017a

narginchk(2,2) ;

if numel(M)>1 || M < 0 || isnan(M) || isinf(M)
    if ~ismatrix(M) || ~any(size(M)==1)
...
if ~isempty(A)   
    if numel(A)==1
        % first input was a negative scalar
        C = zeros(size(B)) ;
        C(B) = A ;
    else
        % vector argument 
