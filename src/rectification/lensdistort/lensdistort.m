function I2 = lensdistort(I, k,principalPoint, varargin)
...
% Nested function that perfoms the transformation
    ...
    brcor = bordercorrect(r,s,k(1), center, R);
...
% Nested function that pics the model type to be used
    function s = distortfun(r,k)
       if(size(k,2)==2) 
            s = r.*(1./(1+k(1).*(r)+k(2).*(r.^2)));
       elseif(size(k,2)==3)
           s = r.*(1./(1+k(1).*(r)+k(2).*(r.^2)+k(3).*(r.^3)));
       else
           error('k needs to be a 1x2 or 1x3 vector with radial distortion parameters');
        end
    end
